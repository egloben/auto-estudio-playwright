...
test.describe("Form Layouts page", () => {
  ...
  test("radio buttons", async ({ page }) => {
    ...
    await usingTheGridEmailForm
      .getByRole("radio", { name: "Option 1" })
      .check({ force: true });

    const radioStatus = await usingTheGridEmailForm
      .getByRole("radio", { name: "Option 1" })
      .isChecked();
    expect(radioStatus).toBeTruthy();
    await expect(
      usingTheGridEmailForm.getByRole("radio", { name: "Option 1" })
    ).toBeChecked();
...
test("tooltips", async ({ page }) => {
  ...
  page.getByRole("tooltip"); //if you hace a role tootip created
...
test("web tables", async ({ page }) => {
  ...
  //2 get the row on the value in the specific columns
...
test("datepicker", async ({ page }) => {
  ...
  let calendarMonthAdnYear = await page
    .locator("nb-calendar-view-mode")
    .textContent();
  const expectedMonthAndYear = ` ${expectedMonthLong} ${expectedYear}`;
  while (!calendarMonthAdnYear.includes(expectedMonthAndYear)) {
    await page
      .locator('nb-calendar-pageable-navigation [data-name="chevron-right"]')
      .click();
    calendarMonthAdnYear = await page
      .locator("nb-calendar-view-mode")
      .textContent();
  }
